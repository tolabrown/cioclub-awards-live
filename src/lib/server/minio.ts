...
/**
 * Upload file and create a record in the database
 */
...
/**
 * Upload buffer and create a record in the database
 */
export async function uploadAndSaveBuffer(
  buffer: Buffer,
  contentType: string,
  originalFilename?: string,
  bucketName: string = BUCKET_NAME,
  ...
  // Generate unique filename if not provided
  const objectName = customObjectName || `${Date.now()}-${originalFilename || 'file'}`;
...
/**
 * Delete file from MinIO and remove record from database
 */
export async function deleteAndRemoveFile(
  dbId: string,
  bucketName: string = BUCKET_NAME
): Promise<void> {
  const fileRecord = await db.query.file.findFirst({
    where: eq(fileTable.id, dbId)
  });

  if (!fileRecord) return;

  // Delete from MinIO
  await deleteFileById(bucketName, fileRecord.remoteId);

  // Delete from database
  await db.delete(fileTable).where(eq(fileTable.id, dbId));
}

/**
 * Upload file and create a record in the album_media table
 */
export async function uploadAndSaveAlbumMedia(
  file: File,
  bucketName: string = BUCKET_NAME,
  customObjectName?: string
): Promise<UploadResult & { dbId: string }> {
  ...
  const [mediaRecord] = await db.insert(albumMediaTable).values({
    remoteId: uploadResult.id,
    url: uploadResult.url,
    size: uploadResult.size,
    type: uploadResult.contentType,
...
/**
 * Delete album media from MinIO and remove record from database
 */
export async function deleteAndRemoveAlbumMedia(
  dbId: string,
  bucketName: string = BUCKET_NAME
): Promise<void> {
  const mediaRecord = await db.query.albumMedia.findFirst({
    where: eq(albumMediaTable.id, dbId)
  });

  if (!mediaRecord) return;

  // Delete from MinIO
  await deleteFileById(bucketName, mediaRecord.remoteId);

  // Delete from database
  await db.delete(albumMediaTable).where(eq(albumMediaTable.id, dbId));
}
