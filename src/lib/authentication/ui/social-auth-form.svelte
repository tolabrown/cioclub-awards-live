<script lang="ts">
	...
	import {
		getRedirectUrl,
		handleSocialSignin,
	} from "$lib/authentication/client";
	interface Props {
		text: string;
	}
...
<div class="flex flex-col gap-4">
	<Button
		variant="outline"
		class="w-full rounded-xl border-border/50 font-bold transition-all gap-3"
		type="button"
		disabled={isLoading || isSocialLoading}
		onclick={handleSocial}
	>
		{#if isSocialLoading}<LoadingSpinner />Loading...{:else}<GoogleIcon
			/>{text}{/if}
	</Button>
</div>
