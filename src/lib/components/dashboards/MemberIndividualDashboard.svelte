...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="relative p-10 md:p-14 flex flex-col md:flex-row items-center justify-between gap-8">
      <div class="space-y-6 text-center md:text-left z-10">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
          ...
          <span class="text-[10px] font-bold uppercase tracking-widest text-white">Individual Membership Active</span>
        </div>
        <h1 class="text-5xl md:text-6xl font-bold tracking-tight text-white leading-tight">
          Members <span class="text-amber-500 italic">Corner</span>
        </h1>
        <p class="text-xl text-gray-400 max-w-xl leading-relaxed">
          Welcome back to your personalized leadership hub. Track your engagement, access exclusive resources, and connect with the African IT elite.
        </p>
        <div class="flex flex-wrap justify-center md:justify-start gap-4 pt-2">
          <Button href="/events" class="rounded-xl px-10 bg-amber-500 hover:bg-amber-600 text-black shadow-xl shadow-amber-500/20">
            Browse Events
          </Button>
      ...
      <div class="relative size-64 hidden lg:flex items-center justify-center">
        ...
        <div class="relative animate-float">
          <Star class="size-48 text-amber-500 drop-shadow-[0_0_30px_rgba(245,158,11,0.4)]" />
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
    ...
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>Everything included in your active plan</Card.Description>
      </Card.Header>
      <Card.Content>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          {#each memberBenefits as benefit}
            <div class="flex items-start gap-4 p-5 rounded-xl border border-border bg-card/50 hover:bg-muted/30 transition-colors group">
              <div class="size-10 rounded-lg bg-primary/10 flex items-center justify-center text-primary shrink-0 group-hover:scale-110 transition-transform">
                <benefit.icon class="size-5" />
              </div>
              <div class="space-y-1">
                <p class="font-bold text-foreground">{benefit.title}</p>
    ...
    {#if engagementData.length > 0}
      <Card.Root>
        <Card.Header>
          ...
          <Card.Description>Activity score across categories</Card.Description>
        </Card.Header>
        <Card.Content class="flex justify-center p-6">
          <ScrollArea class="w-full">
            <Chart.Container config={radarConfig} class="aspect-square max-h-[300px] min-w-[300px] mx-auto">
              <LineChart
                data={engagementData}
                series={[{ key: "value", label: "Score", color: radarConfig.value.color, props: { fill: radarConfig.value.color, fillOpacity: 0.3 } }]}
                radial
                x="metric"
                y="value"
                xScale={scaleBand()}
                ...
                props={{
                  spline: { curve: curveLinearClosed, strokeWidth: 3 },
                  xAxis: { tickLength: -8, stroke: "var(--border)" },
                  yAxis: { format: () => "" },
                  grid: { radialY: "linear", stroke: "var(--border)", strokeDasharray: "4 4" },
                  highlight: { lines: false, points: { r: 5, fill: "white", strokeWidth: 2 } },
                }}
              />
            </Chart.Container>
          </ScrollArea>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="space-y-8">
      <!-- Member Status Badge -->
      <div class="p-8 rounded-2xl border border-primary/20 bg-[#0A0A0A] shadow-xl relative overflow-hidden group">
         <div class="absolute inset-0 bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
         <div class="relative z-10 space-y-4">
            <div class="flex items-center justify-between">
              <p class="text-[10px] font-black tracking-[0.2em] text-primary uppercase">Elite Status</p>
              <Crown class="size-5 text-amber-500 animate-pulse" />
            </div>
            <div>
              <h3 class="text-2xl font-bold text-white mb-1">Peak Performance</h3>
              <p class="text-sm text-gray-400">Your engagement is in the top 10% of the network this month.</p>
            </div>
            <div class="pt-2">
               <div class="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                  <div class="h-full bg-primary w-[85%] rounded-full shadow-[0_0_10px_rgba(var(--primary),0.5)]"></div>
               </div>
            </div>
         </div>
      </div>
      ...
      <Card.Root class="border-border/50">
        <Card.Header>
          <Card.Title class="text-lg font-bold">Support & Help</Card.Title>
          <Card.Description>We're here for you</Card.Description>
        </Card.Header>
        <Card.Content class="space-y-3">
          <Button variant="outline" size="sm" href="/contact" class="w-full justify-start gap-3 h-12 border-border/60 hover:bg-muted font-bold text-sm rounded-xl">
             <HelpCircle class="size-4" /> Help Center
          </Button>
          <Button variant="outline" size="sm" href="/faq" class="w-full justify-start gap-3 h-12 border-border/60 hover:bg-muted font-bold text-sm rounded-xl">
             <Zap class="size-4" /> Common Questions
          </Button>
        </Card.Content>
      </Card.Root>

      <!-- Community Pulse -->
      <div class="p-6 rounded-xl border border-emerald-500/20 bg-emerald-500/5 relative overflow-hidden group transition-all hover:bg-emerald-500/10">
         ...
         <div class="space-y-1">
            <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">Network Pulse</p>
