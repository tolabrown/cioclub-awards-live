<script lang="ts">
  ...
  import SubscriptionCountdown from "./SubscriptionCountdown.svelte";
  ...
  const teamEngagement = [
    { member: "Alice", events: 12 },
    { member: "Bob", events: 8 },
    { member: "Carol", events: 15 },
    { member: "Dan", events: 6 },
    { member: "Eve", events: 10 },
  ];

  const barConfig = {
    events: { label: "Events Attended", color: "var(--chart-1)" },
  } satisfies Chart.ChartConfig;

  type IconComponent = Component<{ class?: string }>;
  type StatItem = { label: string; value: string; icon: IconComponent; trend: string; color: string; bgColor: string; };

  const corporateStats: StatItem[] = [
    { label: "Team Members", value: "12", icon: Users, trend: "+2", color: "text-primary", bgColor: "bg-primary/10" },
    { label: "Events Attended", value: "45", icon: Calendar, trend: "+8", color: "text-emerald-500", bgColor: "bg-emerald-500/10" },
    { label: "Resources", value: "120", icon: BookOpen, trend: "+12", color: "text-sky-500", bgColor: "bg-sky-500/10" },
    { label: "Nominations", value: "8", icon: Award, trend: "+1", color: "text-amber-500", bgColor: "bg-amber-500/10" },
  ];
...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="relative p-10 md:p-14 flex flex-col md:flex-row items-center justify-between gap-8">
      <div class="space-y-6 text-center md:text-left z-10">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
          ...
          <span class="text-[10px] font-bold uppercase tracking-widest text-white">Corporate Membership Active</span>
        </div>
        <h1 class="text-5xl md:text-6xl font-bold tracking-tight text-white leading-tight">
          Organization <span class="text-amber-500 italic">Hub</span>
        </h1>
        <p class="text-xl text-gray-400 max-w-xl leading-relaxed">
          Manage your team's access, track organization-wide engagement, and leverage corporate benefits for your entire IT leadership.
        </p>
        <div class="flex flex-wrap gap-4 mt-8">
							<Button 
								href={page.data.user?.organizationId ? `/corporate/${page.data.user.organizationId}` : "/corporate/new"}
								class="rounded-xl bg-orange-500 hover:bg-orange-600 text-white shadow-xl shadow-orange-500/20"
							>
								{page.data.user?.organizationId ? "Manage Team" : "Set Up Team"}
							</Button>
							<Button variant="outline" class="rounded-xl bg-white/10 hover:bg-white/20 border-white/20 text-white">
								Corporate Benefits
							</Button>
						</div>
      </div>
      <div class="relative size-64 hidden lg:flex items-center justify-center">
        ...
        <div class="relative animate-float">
          <Building2 class="size-48 text-amber-500 drop-shadow-[0_0_30px_rgba(245,158,11,0.4)]" />
        </div>
      </div>
    </div>
  </div>
  ...
  <SubscriptionCountdown subscriptionEndsAt={page.data.user?.subscriptionEndsAt} />

  <!-- Stats Grid -->
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4 lg:grid-cols-4">
    {#each corporateStats as stat}
      <Card.Root>
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
      ...
      <Card.Root>
        <Card.Header>
          ...
          <Card.Description>Events attended per team member</Card.Description>
        </Card.Header>
        <Card.Content>
          <ScrollArea class="w-full">
            <Chart.Container config={barConfig} class="h-[250px] w-full min-w-[400px]">
              <BarChart
                ...
                axis="x"
                series={[{ key: "events", label: "Events", color: barConfig.events.color }]}
                props={{
                  bars: {
                    stroke: "none",
                    rounded: "all",
      ...
      <!-- Add Team Members CTA -->
      <Card.Root class="bg-primary/5 border-primary/10 shadow-lg relative overflow-hidden flex flex-col justify-center">
        <Card.Content class="p-10 space-y-6">
          <div class="flex items-center gap-6">
            <div class="size-20 rounded-2xl bg-primary/20 flex items-center justify-center text-primary">
              <UserPlus class="size-10" />
            </div>
            <div class="space-y-2">
              <h3 class="text-2xl font-bold text-foreground">Expand Your Team</h3>
              <p class="text-muted-foreground leading-relaxed">Invite more colleagues to leverage your corporate membership benefits.</p>
            </div>
          </div>
          					<Button 
						href={page.data.user?.organizationId ? `/corporate/${page.data.user.organizationId}` : "/corporate/new"} 
						class="w-full rounded-xl shadow-xl shadow-primary/20"
					>
						{page.data.user?.organizationId ? "Manage Team Access" : "Set Up Organization"}
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card.Header class="bg-muted/30 pb-4 border-b text-foreground">
          <div class="flex items-center justify-between">
            <div class="space-y-1">
              ...
              <Card.Description>High-impact gatherings for your team</Card.Description>
            </div>
              ...
              <div class="group relative overflow-hidden rounded-xl border border-border p-4 transition-all bg-card shadow-sm">
                <div class="flex items-start gap-4">
                  ...
                  <div class="flex-1 min-w-0 text-foreground">
                    ...
                    <div class="mt-3 flex items-center justify-between"><Badge variant="secondary" class="text-[10px] font-bold tracking-tighter uppercase">{event.type}</Badge></div>
                  </div>
                </div>
              </div>
            {:else}
              <div class="col-span-full p-12 text-center space-y-3">
                ...
                <p class="text-muted-foreground font-medium">Stay tuned for upcoming networking events!</p>
              </div>
            {/each}
          </div>
        </Card.Content>
    ...
    <div class="space-y-8">
      ...
      <Card.Root class="bg-primary/5 border-primary/10 shadow-lg overflow-hidden relative">
        ...
        <Card.Header>
          <Card.Title class="text-xl font-bold">Corporate Benefits</Card.Title>
          <Card.Description>Maximize organization value</Card.Description>
        </Card.Header>
        <Card.Content class="space-y-6">
          ...
          <Button href="/membership" class="w-full font-bold shadow-md shadow-primary/20 rounded-xl">Explore All Hubs</Button>
        </Card.Content>
      </Card.Root>
      ...
      <Card.Root class="border-border/50">
        ...
        <Card.Content class="space-y-4">
          <p class="text-sm text-muted-foreground">Our corporate account managers are available to assist with team onboarding.</p>
          <div class="flex flex-col gap-2">
             <Button variant="outline" size="sm" href="/contact" class="justify-start gap-2 h-11 border-border/60 hover:bg-muted font-medium text-foreground">
               <Building2 class="size-4" /> Account Manager
             </Button>
             <Button variant="outline" size="sm" href="/faq" class="justify-start gap-2 h-11 border-border/60 hover:bg-muted font-medium text-foreground">
               <HelpCircle class="size-4" /> Corporate FAQs
             </Button>
          </div>
        </Card.Content>
      </Card.Root>
