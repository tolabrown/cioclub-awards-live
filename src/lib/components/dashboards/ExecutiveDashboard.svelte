<script lang="ts">
  ...
  const industryTrends = [
    { date: new Date("2024-01-01"), index: 45 },
    { date: new Date("2024-02-01"), index: 52 },
    { date: new Date("2024-03-01"), index: 58 },
    { date: new Date("2024-04-01"), index: 65 },
    { date: new Date("2024-05-01"), index: 72 },
    { date: new Date("2024-06-01"), index: 78 },
  ];

  const focusAreas = [
    { area: "Digital Transformation", value: 35, color: "var(--chart-1)" },
    { area: "Cybersecurity", value: 28, color: "var(--chart-2)" },
    { area: "Cloud Strategy", value: 22, color: "var(--chart-3)" },
    { area: "AI/ML", value: 15, color: "var(--chart-4)" },
  ];

  const lineConfig = {
    index: { label: "Trend Index", color: "var(--chart-1)" },
  ...
  const pieConfig = {
    ...
    Cybersecurity: { label: "Cybersecurity", color: "var(--chart-2)" },
    "Cloud Strategy": { label: "Cloud Strategy", color: "var(--chart-3)" },
    "AI/ML": { label: "AI/ML", color: "var(--chart-4)" },
  } satisfies Chart.ChartConfig;

  type IconComponent = Component<{ class?: string }>;
  type StatItem = { label: string; value: string; icon: IconComponent; trend: string; color: string; bgColor: string; };

  const executiveStats: StatItem[] = [
    { label: "Board Meetings", value: "24", icon: Users, trend: "+2", color: "text-amber-500", bgColor: "bg-amber-500/10" },
    { label: "Strategic Events", value: "12", icon: Calendar, trend: "+1", color: "text-primary", bgColor: "bg-primary/10" },
    { label: "Industry Reports", value: "48", icon: Target, trend: "+5", color: "text-emerald-500", bgColor: "bg-emerald-500/10" },
    { label: "Global Network", value: "500+", icon: Globe, trend: "+50", color: "text-sky-500", bgColor: "bg-sky-500/10" },
  ];
...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="relative p-10 md:p-14 flex flex-col md:flex-row items-center justify-between gap-8">
      <div class="space-y-6 text-center md:text-left z-10">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
          <Crown class="size-4" />
          <span class="text-[10px] font-bold uppercase tracking-widest text-white">Executive Suite Access</span>
        </div>
        <h1 class="text-5xl md:text-6xl font-bold tracking-tight text-white leading-tight">
          Strategic <span class="text-amber-500 italic">Insights</span>
        </h1>
        <p class="text-xl text-gray-400 max-w-xl leading-relaxed">
          Unlock exclusive industry reports, high-level networking opportunities, and strategic oversight for the future of IT.
        </p>
  ...
  {#if industryTrends.length > 0 || focusAreas.length > 0}
      ...
      {#if industryTrends.length > 0}
        <Card.Root>
          <Card.Header>
            ...
            <Card.Description>Market direction over time</Card.Description>
          </Card.Header>
          <Card.Content>
            <ScrollArea class="w-full">
              <Chart.Container config={lineConfig} class="h-[250px] w-full min-w-[400px]">
                <LineChart
                  ...
                  xScale={scaleUtc()}
                  axis="x"
                  series={[{ key: "index", label: "Index", color: lineConfig.index.color }]}
                  props={{
                    spline: { curve: curveNatural, strokeWidth: 2 },
                    xAxis: { format: (v: any) => v.toLocaleDateString("en-US", { month: "short" }) },
      ...
      {#if focusAreas.length > 0}
        <Card.Root>
          <Card.Header>
            ...
            <Card.Description>Industry priorities</Card.Description>
          </Card.Header>
          <Card.Content class="flex justify-center">
            <Chart.Container config={pieConfig} class="aspect-square max-h-[250px]">
              <PieChart data={focusAreas} key="area" value="value" c="color" padding={{ top: 20, right: 20, bottom: 20, left: 20 }} />
            </Chart.Container>
          </Card.Content>
        </Card.Root>
      {/if}
    ...
    <div class="space-y-8">
      ...
      <Card.Root class="border-border/50">
        ...
        <Card.Content class="space-y-4">
          <p class="text-sm text-muted-foreground">Our concierge team is available to assist with your executive requirements.</p>
          <div class="flex flex-col gap-2">
             <Button variant="outline" size="sm" href="/contact" class="justify-start gap-2 h-11 border-border/60 hover:bg-muted font-medium text-foreground">
               <Briefcase class="size-4" /> Executive Concierge
             </Button>
             <Button variant="outline" size="sm" href="/faq" class="justify-start gap-2 h-11 border-border/60 hover:bg-muted font-medium text-foreground">
               <HelpCircle class="size-4" /> Strategic FAQs
             </Button>
          </div>
        </Card.Content>
      </Card.Root>
