<script lang="ts">
  ...
  const quickLinks = [
    { label: "Awards Hub", href: "/awards", icon: Trophy, color: "text-amber-500", bgColor: "bg-amber-500/10" },
    { label: "Latest News", href: "/news", icon: Newspaper, color: "text-sky-500", bgColor: "bg-sky-500/10" },
    { label: "Events", href: "/events", icon: Calendar, color: "text-primary", bgColor: "bg-primary/10" },
    { label: "Our Team", href: "/team", icon: Users, color: "text-violet-500", bgColor: "bg-violet-500/10" },
    { label: "Resources", href: "/resources", icon: Briefcase, color: "text-emerald-500", bgColor: "bg-emerald-500/10" },
    { label: "Membership", href: "/membership", icon: Crown, color: "text-rose-500", bgColor: "bg-rose-500/10" },
  ];

  const memberBenefits = [
    {
      title: "Exclusive Events",
      description: "Access member-only gatherings",
      icon: Calendar,
    },
    {
      title: "Elite Insights",
      description: "Expert reports and analysis",
      icon: Sparkles,
    },
    {
      title: "Networking",
      description: "Connect with IT leaders",
      icon: Users,
    },
...
<div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="relative p-10 md:p-14 flex flex-col md:flex-row items-center justify-between gap-8">
      <div class="space-y-6 text-center md:text-left z-10">
        <h1 class="text-5xl md:text-6xl font-bold tracking-tight text-white leading-tight">
          Welcome to your <span class="text-amber-500">Member Hub</span>
        </h1>
        <p class="text-xl text-gray-400 max-w-xl leading-relaxed">
          Everything you need to stay ahead in Africa's digital landscape. Access exclusive events, insights, and a community of elite IT leaders.
        </p>
      ...
      <div class="relative size-64 hidden lg:flex items-center justify-center">
        ...
        <div class="relative animate-float">
          <Crown class="size-48 text-amber-500 drop-shadow-[0_0_30px_rgba(245,158,11,0.4)]" />
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row gap-2 sm:items-center justify-between">
      <h2 class="text-2xl font-bold text-white tracking-tight">Quick Access</h2>
      <p class="text-sm text-gray-500 font-medium">Shortcuts to key platform areas</p>
    </div>
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      {#each quickLinks as link}
        <a 
          href={link.href}
          class="flex flex-col items-center justify-center gap-4 p-8 rounded-2xl border border-white/5 bg-[#141414] hover:bg-[#1A1A1A] transition-all group hover:scale-[1.03] hover:shadow-2xl hover:border-white/10"
        >
          <div class="size-16 rounded-2xl bg-white/[0.03] flex items-center justify-center group-hover:scale-110 transition-transform duration-300 border border-white/5 {link.color}">
            <link.icon class="size-8 stroke-[1.5]" />
          </div>
          <span class="text-[11px] font-black text-white text-center uppercase tracking-[0.15em] opacity-80 group-hover:opacity-100 transition-opacity">{link.label}</span>
        </a>
      {/each}
    </div>
  </div>
      ...
      <Card.Root class="overflow-hidden border-border/50 shadow-md">
        <Card.Header class="bg-muted/30 pb-4 border-b">
          <div class="flex flex-col sm:flex-row gap-2 sm:items-center justify-between">
            ...
            <Button variant="secondary" size="sm" href="/news" class="rounded-lg font-bold">
               View All
            </Button>
          </div>
        </Card.Header>
          ...
          <div class="divide-y divide-border">
            {#each recentNews.slice(0, 4) as article}
              <a
                ...
                class="flex items-center gap-4 p-5 hover:bg-muted/50 transition-colors group"
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="font-bold text-base text-foreground line-clamp-1 group-hover:text-primary transition-colors">
                    {article.title}
                  </p>
      ...
      <Card.Root class="overflow-hidden border-border/50 shadow-md">
        <Card.Header class="bg-muted/30 pb-4 border-b">
          <div class="flex flex-col sm:flex-row gap-2 sm:items-center justify-between">
            ...
            <Button variant="secondary" size="sm" href="/events" class="rounded-lg font-bold">
               Browse All
            </Button>
          </div>
        </Card.Header>
        <Card.Content class="p-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            {#each upcomingEvents.slice(0, 4) as event}
              <div
                class="group relative overflow-hidden rounded-xl border border-border p-4 transition-all bg-card shadow-sm"
              >
                <div class="flex items-start gap-4">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-muted-foreground flex items-center gap-1 mt-1">
                       <Briefcase class="size-3" /> {event.location || 'Online'}
                    </p>
    ...
    <div class="space-y-8">
      ...
      <Card.Root class="bg-primary/5 border-primary/10 shadow-lg overflow-hidden relative">
        <div class="absolute top-0 right-0 p-4 opacity-10">
           <Trophy class="size-24 rotate-12" />
        </div>
        <Card.Header>
        ...
        <Card.Content class="space-y-6">
          <div class="space-y-4">
            {#each memberBenefits as benefit}
              <div class="flex items-start gap-3 group">
                <div class="size-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary shrink-0 group-hover:scale-110 transition-transform">
                  <benefit.icon class="size-4" />
                </div>
          ...
          <Button href="/membership" class="w-full font-bold shadow-md shadow-primary/20 rounded-xl">
      ...
      <Card.Root class="border-border/50">
        ...
        <Card.Content class="space-y-4">
          <p class="text-sm text-muted-foreground">
             Our community managers are available 24/7 to assist with your journey.
          </p>
          <div class="flex flex-col gap-2">
             <Button variant="outline" size="sm" href="/contact" class="justify-start gap-2 h-11 border-border/60 hover:bg-muted font-medium">
               <Settings class="size-4" /> Support Center
             </Button>
             <Button variant="outline" size="sm" href="/faq" class="justify-start gap-2 h-11 border-border/60 hover:bg-muted font-medium">
      ...
      <div class="p-6 rounded-xl border border-emerald-500/20 bg-emerald-500/5 relative overflow-hidden group">
         ...
         <div class="space-y-1">
            ...
            <p class="text-xs text-muted-foreground">{stats?.totalUsers || 'Significant'} leaders are currently networking.</p>
         </div>
      </div>
    </div>
  </div>
