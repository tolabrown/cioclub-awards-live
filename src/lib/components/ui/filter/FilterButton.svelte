<script lang="ts">
  ...
  const activeCount = $derived(
    filters.filter((f) => {
      if (f.type === "switch") return f.value !== undefined;
...
{#if mounted && Dialog && Drawer}
  ...
  {/if}
{:else}
  <!-- Fallback button when Dialog/Drawer not loaded yet or during SSR -->
  <Button
    variant="outline"
    size={isDesktop ? "sm" : "icon"}
    class={isDesktop ? "h-9 gap-2" : "h-9 w-9 relative"}
    onclick={() => {}}
    disabled
  >
    <Filter class="h-4 w-4" />
    ...
    {#if activeCount > 0}
      <span
        class={isDesktop
          ? "ml-1 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] text-primary-foreground"
          : "absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[10px] text-primary-foreground"}
      >
        {activeCount}
      </span>
    {/if}
