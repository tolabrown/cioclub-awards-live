<script lang="ts">
  import { AwardsHeroContent, getStyle } from "$lib/constants";
  ...
  let { items = AwardsHeroContent } = $props();
  ...
  function startAutoAdvance() {
    ...
    intervalId = setInterval(nextHero, 10000); // 10 seconds per slide
  }
      ...
      <div class="absolute inset-0 z-0" transition:fade={{ duration: 1000 }}>
        ...
        >
          <div class="flex flex-col gap-4 text-center max-w-5xl">
            ...
            <h1
              class="animate-float text-4xl font-bold md:text-6xl lg:text-8xl text-primary tracking-tight"
            >
              {hero.title.split(" ").slice(0, -3).join(" ")}
              <span class="text-primary"
                >{hero.title.split(" ").slice(-3).join(" ")}</span
              >
            </h1>
            <p
            ...
            <div class="flex flex-col justify-center gap-4 sm:flex-row">
              ...
              <Button
                variant="outline"
                class="w-full sm:w-auto border-white/20 text-white bg-transparent transition-all rounded-xl backdrop-blur-md font-bold hover:bg-white/10 px-6"
