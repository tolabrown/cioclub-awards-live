<script lang="ts">
  ...
  let { images, startIndex = $bindable(0), onClose }: Props = $props();
...
>
  ...
  <!-- Header Controls -->
  <div
    class="absolute top-0 left-0 right-0 p-4 md:p-8 z-50 flex items-center justify-between bg-linear-to-b from-black/90 via-black/40 to-transparent"
  >
  ...
  <!-- Thumbnail Strip -->
  <div
    class="absolute bottom-0 left-0 right-0 p-6 flex justify-center items-end gap-3 md:gap-5 overflow-x-auto bg-linear-to-t from-black/95 via-black/40 to-transparent z-30 no-scrollbar pb-10"
  >
    {#each images as img, i}
      <button
