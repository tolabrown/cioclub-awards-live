...
import { updatePartner, deletePartner, getPartnersBySearchFilter } from '$lib/db/crm';

export const PATCH: RequestHandler = async ({ params, request, locals }) => {
  if (!locals.user) return json({ status: 'error', message: 'Unauthorized' }, { status: 401 });
  const body = await request.json();
  const updated = await updatePartner(params.id, body);
...
export const DELETE: RequestHandler = async ({ params, locals }) => {
  if (!locals.user) return json({ status: 'error', message: 'Unauthorized' }, { status: 401 });
  const deleted = await deletePartner(params.id);
