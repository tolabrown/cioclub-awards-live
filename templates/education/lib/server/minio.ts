...
const IS_PROD = ENDPOINT.includes('dhubgroup.com');
console.log(`[MinIO] Initializing with endpoint: ${ENDPOINT} (Prod: ${IS_PROD})`);

const minioClient = new Client({
  endPoint: ENDPOINT,
  // If prod, use port 443 (default). If local, use 9000.
  port: IS_PROD ? undefined : 9000,
  useSSL: IS_PROD,
...
export function getDirectObjectUrl(bucketName: string, objectName: string): string {
  const host = env.MINIO_ENDPOINT || 'api.minio.dhubgroup.com';
  const protocol = host.includes('dhubgroup.com') ? 'https' : 'http';
...
export function getObjectUrl(bucketName: string, objectName: string): string {
  ...
  return `https://${host}/api/v1/buckets/${bucketName}/objects/download?preview=true&prefix=${encodeURIComponent(objectName)}&version_id=null`;
}
