    ...
    <CardHeader class="pb-6 text-center">
      <div class="mb-4 flex justify-center">
        <UserProfileImageCropper
          src={image ? URL.createObjectURL(image) : (user.image as string)}
          {onCropped}
        />
      </div>
    ...
    <CardContent class="space-y-6">
      <div class="space-y-2">
        <Label for="name" class="flex items-center gap-2"
          ><UserIcon class="h-4 w-4" /> Name</Label
        >
        <Input
          id="name"
          name="name"
      ...
      <div class="space-y-2">
        <Label for="email" class="flex items-center gap-2"
          ><Mail class="h-4 w-4" /> Email</Label
        >
        <Input
          id="email"
          type="email"
      ...
      <div class="pt-4">
        <Button type="submit" disabled={saving} class="w-full sm:w-fit">
          {#if saving}<LoadingSpinner class="text-white dark:text-background" /> Saving...{:else}Save
            Changes{/if}
        </Button>
      </div>
    </CardContent>
