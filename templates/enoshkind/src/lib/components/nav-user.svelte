		...
		<DropdownMenu.Root>
			...
			>
				...
				<DropdownMenu.Group class="flex flex-col gap-1">
					...
					{#each navItems.navMain as group}
						{@const firstAccessibleItem = group.items.find((item) =>
							item.roles.includes(user.role as Role)
						)}
						{#if firstAccessibleItem}
							<DropdownMenu.Item
								class={cn(
									buttonVariants({ variant: firstAccessibleItem.isActive ? 'outline' : 'ghost' }),
									'cursor-pointer justify-start'
								)}
								onclick={() => (location.href = firstAccessibleItem.url)}
							>
								<firstAccessibleItem.icon />
								{firstAccessibleItem.title}
							</DropdownMenu.Item>
						{/if}
					{/each}
				</DropdownMenu.Group>
