...
import { MAX_ITEMS_PER_PAGE } from '$lib/constants'; // Ensure MAX_ITEMS_PER_PAGE is generic if missing
...
// Fallback constant if not in global constants
...
export class InfiniteScroll {
  ...
  static async fetchList(endpoint: string, offset: number = 0, search: string = '', params?: Record<string, string>) {
    ...
    const metalist: iFetchMeta = { ...partial, lastIndex: offset * ITEMS_PER_PAGE }
    return metalist
  }
  ...
  listQueryAlternate<T>(params: Record<string, any>) {
    const self = this
    const { field, search, userId, ...rest } = params
    return createInfiniteQuery({
      queryKey: [field, search, userId || '', rest],
      staleTime: 30000,
      initialPageParam: 1,
      retry: false, // Disable retries to prevent multiple calls
      queryFn: async (args) => {
        const { pageParam } = args;
        let offset: any = (pageParam - 1) * ITEMS_PER_PAGE;
