<script lang="ts">
	...
	let bannerPreview = $state<string | null>(
		untrack(() =>
			data.article.image && typeof data.article.image === 'object' ? data.article.image.url : null
		)
	);
	let imageId = $state<string | null>(
		untrack(() =>
			data.article.image && typeof data.article.image === 'object' ? data.article.image.id : null
		)
	);
	...
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-4">
			...
			<div>
				...
				<p class="text-muted-foreground font-medium mt-1">Updating "{data.article.title}".</p>
			</div>
		</div>
	</div>
		...
		use:enhance={() => {
			...
			return async ({ result }) => {
				...
				if (result.type === 'redirect') {
					...
					toast.error((result.data as any)?.message || 'Failed to update article');
				}
			};
		}}
						...
						<div class="grid grid-cols-2 gap-4">
							...
							<div class="space-y-4 pt-8">
								<div class="flex items-center justify-between">
									...
									<Switch name="premium" value="true" checked={data.article.premium as boolean} />
								</div>
							</div>
						</div>
					</div>
