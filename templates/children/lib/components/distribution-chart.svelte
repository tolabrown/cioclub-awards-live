...
<Card.Root class="flex flex-col border-0 shadow-none">
  <!-- <Card.Header class="items-center pb-0">
    <Card.Title>{title}</Card.Title>
    <Card.Description>{description}</Card.Description>
  </Card.Header> -->
  <Card.Content class="pb-0 p-0">
          ...
          {#if chartData.length > 0}
            ...
            >
              ...
              {#snippet arc({ props, visibleData, index })}
                {@const value = visibleData[index].value}
                <Arc {...props}>
                  {#snippet children({ getArcTextProps })}
                    <Text
                      ...
                      font-size="16"
                      class="fill-background dark:fill-white font-bold"
                    />
                  {/snippet}
                </Arc>
              {/snippet}
              ...
              <TableBody>
                {#each chartData as item, index}
                  <TableRow>
                    ...
                    <TableCell class="text-right py-2">{item.value}</TableCell>
                  </TableRow>
                {/each}
              </TableBody>
            </Table>
