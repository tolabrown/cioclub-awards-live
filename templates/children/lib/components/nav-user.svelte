		...
		<DropdownMenu.Root>
			...
			>
				...
				<DropdownMenu.Group class="flex flex-col gap-1">
					...
					<!-- Show only the first accessible link from each group -->
					{#each navItems.navGroups as group}
						{@const firstVisibleItem = group.items.find((item) =>
						...
						{#if firstVisibleItem}
							<DropdownMenu.Item
								class={cn(
									buttonVariants({
										variant: firstVisibleItem.isActive ? "outline" : "ghost",
									}),
									"cursor-pointer justify-start",
								)}
								onclick={() => (location.href = firstVisibleItem.url)}
							>
								<firstVisibleItem.icon />
