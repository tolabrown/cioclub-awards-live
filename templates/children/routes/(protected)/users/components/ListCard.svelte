...
{#if screen === "desktop"}
	{@render Desktop(num, user, roleVariant, onDelete, () => {})}
{:else}
	{@render Mobile(num, user, roleVariant, onDelete)}
{/if}
...
{#snippet Desktop(
	...
	<TableRow class="cursor-pointer hover:bg-muted/50" onclick={onClick}>
		...
		<TableCell>
			<Badge variant={roleVariant} class="capitalize">
				{user.role}
			</Badge>
		</TableCell>
		<TableCell>
...
{#snippet Mobile(
	...
	<div class="rounded-lg border bg-white shadow-sm dark:bg-secondary/50">
			...
			<div class="flex items-start justify-between gap-2">
				<div class="min-w-0 flex-1">
					...
					<div class="mt-1 flex flex-wrap items-center gap-1.5">
						...
						<Badge variant={roleVariant} class="text-xs capitalize">
							{user.role}
						</Badge>
					</div>
				</div>
		...
		<!-- Action Footer -->
		<div
			class="flex items-center justify-between border-t bg-muted/30 px-3 py-2"
		>
			{#if isAdmin && me.id !== user?.id}
				<SelectComponent
					disabled={updating === user.id}
