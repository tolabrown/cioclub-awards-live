...
<div class="flex flex-col gap-4">
	...
	<div class="flex flex-col items-center justify-between gap-4 md:flex-row">
		<div class="relative grid w-full sm:max-w-md">
			...
			/>
			<Input
				placeholder="Search users by year, half, or dates..."
		...
		<!-- <Button
			...
			class="flex w-full cursor-pointer items-center gap-2 sm:w-auto"
		>
			<Plus class="h-4 w-4" />
			Add User
		</Button> -->
	...
	{#if $usersQuery.error}
		<Alert.Root variant="destructive">
			<AlertCircleIcon />
			<Alert.Title>
				{$usersQuery.error.name}</Alert.Title
			>
			<Alert.Description>
				{$usersQuery.error.cause}<br />
