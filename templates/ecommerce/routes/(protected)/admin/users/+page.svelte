...
<div class="flex flex-col gap-4">
  <div class="flex flex-col gap-4">
    <h1 class="text-2xl font-bold sm:text-3xl">
      Users ({#if $usersQuery.isPending}<Loader2
  ...
  {#if $usersQuery.isPending}
    <div class="p-6">
      <div class="flex h-80 items-center justify-center">
        <div
          class="size-20 animate-spin rounded-full border-b-2 border-primary"
        ></div>
      </div>
    </div>
  {/if}
  ...
  {#if $usersQuery.error}
    <Alert.Root variant="destructive">
      ...
      <Alert.Description
        >{$usersQuery.error.cause}<br />{$usersQuery.error
  ...
  {#if $usersQuery.isSuccess}
    <div>
      <!-- desktop table -->
      <div
        class="hidden overflow-x-auto border bg-white lg:block dark:bg-background rounded-xl"
      >
        <Table>
          <TableHeader
    ...
    <div class="mt-4">
      ...
      {:else if $usersQuery.hasNextPage}
        <div class="justify-center">
          <Button
            class="w-full sm:w-fit rounded-xl"
            onclick={() => $usersQuery.fetchNextPage()}
            disabled={!$usersQuery.hasNextPage ||
              $usersQuery.isFetchingNextPage}>Load More</Button
          >
        </div>
      {:else}
