<script lang="ts">
  ...
  $effect(() => {
    if (category) {
      ...
      const existingImage =
        typeof category.imageFile === "object" ? category.imageFile : null;
      if (existingImage && !imagePreview && !imageFile) {
  ...
  const removeImage = async () => {
    ...
    if (confirm("Are you sure you want to delete this image?")) {
      isDeleting = true;
      try {
        // If it's an existing image, we use the form action which is handled separately in the template
        // This function is mainly for "unsaved" images or fallback
        const formData = new FormData();
        ...
        if (result.type === "success") {
          ...
          // @ts-ignore
          const errorMsg = (result.data as any)?.error || "Failed to delete image";
          toast.error(errorMsg);
        }
      } catch (error) {
  ...
  // Check if the current image is the one from the database
  const isExistingImage = $derived(
    imagePreview !== null &&
      imagePreview ===
        (typeof category?.imageFile === "object"
          ? category?.imageFile?.url
          : null),
  );
</script>
  ...
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="text-sm text-muted-foreground">
        Update category details and rich visuals
      </p>
    </div>
  </div>
    ...
    use:enhance={() => {
      ...
      return async ({ result }) => {
        ...
        if (result.type === "redirect") {
          ...
          toast.error(
            (result as any).data?.error || "Failed to update category",
          );
        }
      };
    }}
        ...
        <CardContent class="space-y-6">
          ...
          {#if imagePreview}
            ...
            >
              ...
              {#if isExistingImage}
                <div
                  class="absolute top-2 left-2 px-2 py-1 bg-primary text-primary-foreground text-[10px] font-bold uppercase tracking-wider rounded-md shadow-sm"
                >
                  Saved
                </div>
              {/if}
