<script lang="ts">
  ...
  $effect(() => {
    if (data.product) {
      ...
      basePrice = String(data.product.basePrice);
      compareAtPrice = data.product.compareAtPrice
        ? String(data.product.compareAtPrice)
        : "";
      marketPrice = data.product.marketPrice
        ? String(data.product.marketPrice)
        : "";
      stockQuantity = String(data.product.stockQuantity);
      lowStockThreshold = String(data.product.lowStockThreshold);
      ...
      selectedTagIds = data.product.tags?.map((t: any) => t.tagId) || [];
      features = data.product.features || [];
      productSizes =
        data.product.sizes?.map((ps: any) => ({
          sizeId: ps.sizeId,
          additionalPrice: String(ps.additionalPrice),
          stockQuantity: String(ps.stockQuantity),
          sku: ps.sku || "",
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-sm text-muted-foreground">
          Update the details for <strong>{name}</strong>
        </p>
      </div>
    </div>
  </div>
    ...
    <div class="space-y-6 lg:col-span-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription
            >Update your product's story and details</CardDescription
          >
        </CardHeader>
            ...
            <div class="space-y-3">
              {#each features as feature, i}
                <div class="flex gap-3 items-start group">
                  ...
                  <div class="flex-1">
                    <Input bind:value={feature.value} placeholder="Value" />
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
          ...
          <div class="flex gap-2">
            {#if data.sizes.length > 0}
              <div class="group relative">
                ...
                <div
                  class="absolute right-0 top-full mt-1 w-48 bg-popover border rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 divide-y overflow-hidden text-popover-foreground"
                >
