<script lang="ts">
	...
	let lastAppliedValue = $state("");

	// Handle initial and external value updates
	$effect(() => {
		if (composer && value !== lastAppliedValue) {
			const editor = composer.getEditor() as LexicalEditor;

			// Check if the change is external (not from the editor itself)
			let currentHtml = "";
			editor.read(() => {
	...
	// Listen for internal changes and update value
	$effect(() => {
		if (!composer) return;
