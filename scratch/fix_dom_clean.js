...
async function fixKindleCreatePerfection() {
  console.log("Fixing all Kindle Create misalignments cleanly...");
  ...
  let pNodes = Array.from(bodyNode.childNodes).filter(n => n.nodeName === 'w:p');
  ...
  for (let i = 0; i < pNodes.length; i++) {
    ...
    // 1. Remove empty spacer paragraphs
    if (!text) {
    ...
    // 2. Remove duplicate banner/heading callout text
    if (text && text === lastSeenText && text.length > 10 && !/^(Chapter|\d+$)/i.test(text)) {
      pNode.parentNode.removeChild(pNode);
      continue;
    }
    ...
    // 6. Front Matter Section Headings
    ...
    // 7. Chapter Starts (Chapters 1 .. 13)
    let chapterMatch = text.match(/^CHAPTER\s*(\d+)/i);
    if (chapterMatch && i > 250) {
      ...
      if (chInfo) {
        ...
        processedChapters.add(chNum);
        const pb = createPageBreakNode(doc);
        pNode.parentNode.insertBefore(pb, pNode);
        setParagraphStyle(pNode, 'Heading1', doc);
        setNodeText(pNode, chInfo.title, doc);

        // Delete duplicate title subheader lines following
    ...
    // 8. Back Matter Section Headings
    ...
    // 9. For all OTHER paragraphs: demote Heading2/Heading3/Title to Normal so Kindle Create ignores them as suggested chapters!
    ...
    // 10. Strip accidental numPr (bullet list) from regular text
