...
const OUTPUT_16X9 = "C:\\Users\\TOLAN\\Downloads\\Olaniyi_Birthday_Reflection_PERFECT_16x9.mp4";
const OUTPUT_9X16 = "C:\\Users\\TOLAN\\Downloads\\Olaniyi_Birthday_Reflection_PERFECT_9x16.mp4";

// 1. OPENING & MIDDLE IMAGES (Brightsteps Kids & Community Event Photos)
const IMG_EVENT_1 = Path.join(FOLDER, "WhatsApp Image 2026-07-19 at 9.18.49 PM (2).jpeg"); // Brightsteps event close-up
const IMG_EVENT_2 = Path.join(FOLDER, "WhatsApp Image 2026-07-19 at 9.18.49 PM.jpeg");     // African kids learning
const IMG_EVENT_3 = Path.join(FOLDER, "WhatsApp Image 2026-07-19 at 9.18.50 PM (7).jpeg"); // Event attendees & partners
const IMG_EVENT_4 = Path.join(FOLDER, "WhatsApp Image 2026-07-19 at 9.18.50 PM (8).jpeg"); // Smiling child event photo
const IMG_EVENT_5 = Path.join(FOLDER, "WhatsApp Image 2026-07-19 at 9.18.50 PM (1).jpeg"); // Community engagement photo

// 2. THE 3 SPECIFIED ENDING IMAGES:
// Ending Image A: Church bowing handshake in front of pulpit
const IMG_END_CHURCH_BOW = Path.join(FOLDER, "WhatsApp Image 2026-07-19 at 9.18.50 PM (2).jpeg");

// Ending Image B: Church standing prayer in front of pulpit
const IMG_END_CHURCH_PRAY = Path.join(FOLDER, "WhatsApp Image 2026-07-19 at 9.18.50 PM (5).jpeg");

// Ending Image C: Black top with yellow eyes design portrait
const IMG_END_BLACK_YELLOW_TOP = Path.join(FOLDER, "WhatsApp Image 2026-07-19 at 9.18.49 PM (1).jpeg");

const SLIDES = [
  { img: IMG_EVENT_1, dur: 15 },
  { img: IMG_EVENT_2, dur: 15 },
  { img: IMG_EVENT_3, dur: 20 },
  { img: IMG_EVENT_4, dur: 20 },
  { img: IMG_EVENT_5, dur: 15 },
  // ENDING 3 IMAGES IN EXACT SEQUENCE:
  { img: IMG_END_CHURCH_BOW, dur: 17 },      // Church bowing handshake at pulpit
  { img: IMG_END_CHURCH_PRAY, dur: 14 },     // Church standing prayer at pulpit
...
function generateAssFile(isLandscape) {
  ...
  const fontSize = isLandscape ? 52 : 42;  // INCREASED FONT SIZE FOR MAXIMUM READABILITY!
  const titleSize = isLandscape ? 68 : 56;

  // MarginV=35 for lower center placement so images remain 100% clear above!
...
async function prepareAndRender() {
  console.log("Pre-compositing slides with compact dark box for maximum image visibility...");
  ...
  for (let i = 0; i < SLIDES.length; i++) {
    const s = SLIDES[i];
    const out16x9 = `C:\\Users\\TOLAN\\Music\\cioclub_awards\\scratch\\perfect_slide_16x9_${i}.jpg`;
    const out9x16 = `C:\\Users\\TOLAN\\Music\\cioclub_awards\\scratch\\perfect_slide_9x16_${i}.jpg`;

    // 16:9 composite (blurred background + uncropped centered foreground)
    ...
    execSync(`"${FFMPEG_BIN}" -y -i "${s.img}" -vf ${vf16x9} "${out16x9}"`, { stdio: 'pipe' });
    prepped16x9.push({ img: out16x9, dur: s.dur });
    ...
    execSync(`"${FFMPEG_BIN}" -y -i "${s.img}" -vf ${vf9x16} "${out9x16}"`, { stdio: 'pipe' });
    prepped9x16.push({ img: out9x16, dur: s.dur });
  }

  console.log("Pre-compositing finished! Rendering final perfect videos...");

  const ass16x9 = generateAssFile(true);
  const ass9x16 = generateAssFile(false);
  const totalDur = SLIDES.reduce((acc, s) => acc + s.dur, 0); // 132s

  const escapedAss16x9 = ass16x9.replace(/\\/g, '/').replace('C:', 'C\\:');
  const escapedAss9x16 = ass9x16.replace(/\\/g, '/').replace('C:', 'C\\:');

  // Render 16:9 with compact lower-third glass box (y=620, h=440) for maximum image visibility above!
  let input16 = '';
  ...
  console.log("Encoding 16:9 PERFECT Video...");
  execSync(cmd16x9, { stdio: 'inherit' });
  console.log(`16:9 PERFECT Video completed: ${OUTPUT_16X9}`);

  // Render 9:16 with compact lower-third glass box (y=1120, h=680)
  let input9 = '';
  ...
  console.log("Encoding 9:16 PERFECT Video...");
  execSync(cmd9x16, { stdio: 'inherit' });
  console.log(`9:16 PERFECT Video completed: ${OUTPUT_9X16}`);
}
