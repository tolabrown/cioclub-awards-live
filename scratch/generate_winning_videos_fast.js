...
async function renderWinningVideos() {
  console.log("Generating FAST WINNING Video Render (No Faces Cut Off, Full Colors, Fast Blur)...");
  ...
  // Ultra-fast blurred background filter graph (scaling down to low-res and back up)
  let filter16x9 = '';
  SLIDES.forEach((s, idx) => {
    filter16x9 += `[${idx}:v]scale=160:90:force_original_aspect_ratio=increase,crop=160:90,scale=1920:1080[bg${idx}]; `;
  ...
  const cmd16x9 = `"${FFMPEG_BIN}" -y ${inputArgs}-filter_complex "${filter16x9}${concat16x9}" -map "[v]" -map ${SLIDES.length}:a -t ${totalDur} -c:v libx264 -preset ultrafast -crf 19 -c:a aac -b:a 192k "${OUTPUT_16X9}"`;
  ...
  // 9:16 Vertical Mobile filter graph
  let filter9x16 = '';
  SLIDES.forEach((s, idx) => {
    filter9x16 += `[${idx}:v]scale=90:160:force_original_aspect_ratio=increase,crop=90:160,scale=1080:1920[bg${idx}]; `;
  ...
  const cmd9x16 = `"${FFMPEG_BIN}" -y ${inputArgs}-filter_complex "${filter9x16}${concat9x16}" -map "[v]" -map ${SLIDES.length}:a -t ${totalDur} -c:v libx264 -preset ultrafast -crf 19 -c:a aac -b:a 192k "${OUTPUT_9X16}"`;
